...
extern std::map<std::string, std::function<void()>> SLROSCallbackMap;
...
class SimulinkServiceServer {
    public:
        ...
        void setAsyncEnabled(std::string asyncSetting, std::string partitionName){
            if(asyncSetting == "on"){
                mAsyncEnabled = true;

                auto it = SLROSCallbackMap.find(partitionName);
                if (it != SLROSCallbackMap.end()) {
                    mCallbackFunc = it->second;
                    #ifdef MW_DEBUG_LOG
                    RCLCPP_INFO(SLROSNodePtr->get_logger(),"Callback for Partition %s is registered",partitionName.c_str());
                    #endif
                } else {
                    #ifdef MW_DEBUG_LOG
                    RCLCPP_INFO(SLROSNodePtr->get_logger(),"Partition %s not found",partitionName.c_str());
                    #endif
                }
            }else{
                mAsyncEnabled = false;
...
void SimulinkServiceServer<SrvType, ReqBusType, RespBusType>::createServiceServer(
    const std::string& serviceName, const rmw_qos_profile_t& qosProfile) {
        ...
        auto serviceCallback = [this](const std::shared_ptr<typename SrvType::Request> reqMsgPtr,
            ...
            _numOfRequests++;
            #ifdef MW_DEBUG_LOG
            RCLCPP_INFO(SLROSNodePtr->get_logger(),"Service: %s, rclcpp callback triggered",mServiceName.c_str());
            #endif
            if (mAsyncEnabled){
                mCallbackFunc();
                #ifdef MW_DEBUG_LOG
                RCLCPP_INFO(SLROSNodePtr->get_logger(),"Service: %s, simulink callback executed",mServiceName.c_str());
                #endif
            }else{
                #ifdef MW_DEBUG_LOG
                RCLCPP_INFO(SLROSNodePtr->get_logger(),"Service: %s, waiting for response to be sent",mServiceName.c_str());
                #endif
                std::unique_lock<std::mutex> lck(_srvMtx);
                _responseRecvCond.wait(lck);
            }
            #ifdef MW_DEBUG_LOG
            RCLCPP_INFO(SLROSNodePtr->get_logger(),"Service: %s, response sent",mServiceName.c_str());
            #endif
        };
...
bool SimulinkServiceServer<SrvType, ReqBusType, RespBusType>::getCurrentRequest(ReqBusType* reqBusPtr){
    ...
    if (_isRequestNew) {
        ...
        #ifdef MW_DEBUG_LOG
        RCLCPP_INFO(SLROSNodePtr->get_logger(),"Service: %s, Request is being processed",mServiceName.c_str());
        #endif
    }
